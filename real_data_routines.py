...
def plot_FRF_cloud_single_sample(
    ...
    with torch.no_grad():
        ...
        ax.plot(frequencies, data_log_mag, label='Input Signal', color='blue')
        for k, omega in enumerate(predicted_omegas):
        ...
        # Optimised results but same scale for presentation by plotting invisible lines
        frequencies = np.linspace(0, 1, data.shape[-1])
        fig, ax = plt.subplots(1, 1, figsize=(8, 5), sharex=True)
        data_log_mag = quick_log_mag(data.cpu().numpy())
        pred_log_mag = quick_log_mag(H_v)
        ax.plot(frequencies, pred_log_mag, label='Model Predicted FRF', color='orange')
        for i in range(0,num_cloud_samples):
            FRF_cloud_log_mag = quick_log_mag(FRF_clouds[i])
            ax.plot(frequencies,FRF_cloud_log_mag, color='red', alpha=0)
        ax.plot(frequencies, data_log_mag, label='Input Signal', color='blue')
        for k, omega in enumerate(predicted_omegas):
            ax.axvline(x=omega, color='cyan', linestyle=':', 
                                label=r'Predicted $\omega_n$' if k == 0 else '')
...
def optimiser_handler(model, data, scale_factors, omega_weight=0, beta=0, plot=True, q=0, window_scale=0.6, up_inc=0.35, min_cut_off=0.8):
    ...
    Omega_percent_bound probably shouldn't be used with omega_weight
    If omega_percent_bound = 0 then omegas are only bounded between 0 and 1
    '''
    with torch.no_grad():
        model.eval()
        output = model(data).squeeze(0)
        ...
        if plot:
            ...
            ax.plot(frequencies, predic_logmag, color='orange', alpha = 0)
