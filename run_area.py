...
data, labels = generate_data(num_samples=4000, signal_length=1024)
train_dataset = simple_dataset(data, labels)
train_dataloader = DataLoader(train_dataset, batch_size=32, shuffle=True)

data, labels = generate_data(num_samples=400, signal_length=1024)
val_dataset = simple_dataset(data, labels)
val_dataloader = DataLoader(val_dataset, batch_size=32, shuffle=True)

plot_samples(train_dataloader, 10)
