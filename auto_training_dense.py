...
max_params = 0
num_models = 0
for bc in block_config_list:
    for ks in kernel_size_list:
        for gr in growth_rate_list:
            for tc in transition_channels_list:
                model = md.DenseNet1(
                    data_channels=int(np.sum(inputs1)),
                    out_channels=int(np.sum(labels1)),
                    growth_rate=gr,
                    block_config=bc,
                    transition_channels=tc,
                    kernel_size=ks,
                )
                num_models += 1
        ...
        for gr in growth_rate_list:
            for tc in transition_channels_list:
                try:
                    ...
                    print(f'Error for ks={ks}, bc={bc}, gr={gr}, tc={tc}: {e}\n')
                    with open(error_log, "a") as log:
                        log.write(f'Error for ks={ks}, bc={bc}, gr={gr}, tc={tc}: {str(e)}\n')
                    continue
